...
*/
  ...
  /* Setting all elements of the array to '*'. */
  memset(&q->arr, '*', sizeof(char) * QUEUE_SIZE);
...
*/
  return (QUEUE_SIZE < 2 || (index != 0 && (index % (QUEUE_SIZE - 1) == 0))) ? 0 : (index + 1);
}}}
...
*/
  ...
  return (q->f_index == q->b_index) ? true : false;
}}}
...
*/
  ...
  return (nextIndex(q->b_index) == q->f_index) ? true : false;
}}}
